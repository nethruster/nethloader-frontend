  ...
  render ({data}) {
    ...
    return (
      <li class={`${style.upload} flex flex-cross-center flex-sb`} data-id={data.id} ref={(el) => { this.uploadEl = el }}>
        <div class={`${style.uploadMedia} flex flex-full-center`} title='Click to open'><Link target='_blank' rel='noopener' to={mediaUrl} class='flex flex-full-center'><AsyncMedia src={mediaPath} type={data.extension} size='74' id={data.id} /></Link></div>
